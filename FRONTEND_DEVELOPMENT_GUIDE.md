...
**구현 시 주요 개선사항**:
1. **관리자 대시보드**: 주요 통계 카드 (사용자, 게시글, 댓글, 신고), 역할별 통계, 포인트 통계 및 상위 사용자
2. **신고 관리**: 필터링 (대기중/승인됨/거부됨/전체), 신고 상세 모달, 처리 메모 기능
3. **역할 기반 접근 제어**: 관리자 레이아웃에서 ADMIN 역할 체크, 미인증 시 로그인 페이지 리다이렉트
4. **다크 모드 지원**: 모든 관리자 페이지에 다크 모드 적용
