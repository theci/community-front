      ...
      <div className="space-y-6 sticky top-20">
          ...
          <nav className="space-y-2">
            {categories.map((category) => (
              <Link
                ...
                className="flex items-center justify-between px-3 py-2 text-gray-700 hover:bg-blue-50 hover:text-blue-600 rounded-lg transition-colors group"
              >
              ...
              {popularPosts.map((post, index) => (
                ...
                >
                  <div className="flex items-start space-x-2">
                    <span className="flex-shrink-0 w-6 h-6 flex items-center justify-center rounded-full bg-blue-100 text-blue-600 text-xs font-semibold">
                      {index + 1}
                    </span>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          {loading ? (
            ...
            <p className="text-sm text-gray-500">트렌딩 게시글이 없습니다.</p>
          )}
        </div>
      </div>
    </aside>
